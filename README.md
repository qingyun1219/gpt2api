...
- **硬错误才切账号**:只有 `rate_limited` / `no_available_account` / `auth_required` 才会触发一次跨账号重试;`content_policy`(上游内容策略拒绝)不会重试(换账号也一样被拒),其他错误直接暴露给调用方便于排障。
...
如果 Poll 超时(默认 300 秒内没拿到任何图),会直接落到 `poll_timeout`,不再悄悄换账号重试。如果上游内容策略拒绝生成(如涉及第三方 IP),Poll 会在 ~30 秒内检测到 assistant 拒绝文本并立即返回 `content_policy`,不再傻等超时。
...
<details>
<summary><b>Q2.1 出图报 content_policy 错误?</b></summary>

这表示 **ChatGPT 上游拒绝了该提示词的图片生成**,通常是因为内容涉及第三方 IP(品牌/角色/名人相似性)。

**工作原理**:SSE 阶段无法区分正常出图和被拒——两者返回的数据完全一样。区别在 Poll 阶段:上游约 20~30 秒后在 conversation mapping 中写入 assistant 纯文本拒绝消息(如 `"非常抱歉,生成的图片可能违反了关于与第三方内容相似性的防护限制"`),Poll 检测到后立即返回 `content_policy` 错误,**不再傻等 300 秒超时**,也**不会重试**(换账号也一样被拒)。

用户侧处理:修改提示词,避免直接提及受保护的品牌/角色名称。
</details>
